...
int main(int argc, char** argv)
{
	...
	EQM_PWR_MLP_MOX* EQMMLPPWRMOX 	= new EQM_PWR_MLP_MOX(gCLASS->GetLog(),PATH_TO_DATA + "PWR/MOX/EQModel/MLP/EQM_MLP_PWR_MOX_3batch.xml");
	...
	/*===A FabricationPlant===*/
	FabricationPlant *FP_MOX = new FabricationPlant(gCLASS->GetLog(), 2*year); //Declare a FabricationPlant. After the build of the fuel, it decays during 3years before to be loaded in Reactor
	...
	double	Power_CPY = 2.785e9/7.;	    //Thermal power (in W)

		//Combustibles type : STANDARD 900
		double  BurnUp_STD900    = 35; 		// GWd/tHM
		double  HMMass_STD900    = 72.3/7.;		//heavy metal mass (in tons)


				//the PWR MOX
				StartingTime =  1995*year;
