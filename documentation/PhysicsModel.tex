...
The CLASS package contains, at the moment, 5 different equivalence models where three are related to the building of fuels for a PWR-MOX , one to the building of  PWR-UOX fuels and one for the FBR-Na MOX:
...
\subsubsection{Baker \& Ross Model: EQM\_BakerRoss\_FBR\_MOX}
...
\begin{minipage}{\textwidth}
\textbf{Implementation in a .cxx }
\begin{lstlisting}[label=lst:IMP_EQMBER,caption=Equivalence Model EQM\_BakerRoss\_FBR\_MOX ]
...
#include "Equivalence/EQM_BakerRoss_FBR_MOX.hxx"
...
int main()
{
...
EQM_BakerRoss_FBR_MOX* Equivalence = new	EQM_BakerRoss_FBR_MOX( ); //the default weight and Eref are used
// or
 EQM_BakerRoss_FBR_MOX* Equivalence = new EQM_BakerRoss_FBR_MOX( Weight_U_235,  Weight_Pu_238,  Weight_Pu_240, Weight_Pu_241, Weight_Pu_242, Weight_Am_241, Eref); 
;
...
}
\end{lstlisting}
...
\begin{minipage}{\textwidth}
\textbf{Implementation in a .cxx : }
\begin{lstlisting}[label=lst:IMP_XSMCLOSEST,caption=Cross section Model XSM\_CLOSEST ]
...
...
\begin{minipage}{\textwidth}
\textbf{Implementation in a .cxx : }
\begin{lstlisting}[label=lst:IMP_XSMMLP,caption=Cross section Model XSM\_MLP ]
...
#include "XS/XSM_MLP.hxx"
...
int main()
...
\end{minipage}
\end{center}
\textbf{PathToWeightFolder} (string) is the path to the folder containing the weight files (.xml files). \textbf{OneMLPPerTime} is a boolean setted to true if there is one MLP per reaction and per time step. \textbf{InfoFileName} (string) is the name of the file located in PathToWeightFolder which is informing on the reactor and on the inputs of the XS\_MLP model. Its default name is Data\_Base\_Info.nfo .
...
"z a i min max" //minimal and maximal proportion of the zai in the fresh fuel (heavy nuclei only, ie without oxygen)"
"z2 a2 i2 min2 max2"
\end{lstlisting}
\end{minipage}
\end{center}
