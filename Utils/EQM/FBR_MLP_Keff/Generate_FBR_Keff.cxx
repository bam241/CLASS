...
//@author BaM, BaL
/**********************************************************/
#include "include/Generate_FBR_Keff.hxx"
#include <TH1F.h>
#include <TH2D.h>
#include <TFile.h>
#include <TTree.h>
...
int main(int argc, char ** argv)
{
	...
	if(is_file_exist("Training_output_k_eff.root" ))
	{
	...
	cout << endl;
	cout << "│                   SELECT YOUR TIME STEP                   │" << endl; 
	cout << "This model aims to predict the fissile content needed to reach\nthe user define keff at a given time (often 0 or end of cycle)"<< endl;
	cout << "Please select the time \033[36m Step \033[0m corresponding to this time" <<endl;
	cout << "\tStep\ttime[year]" <<endl;
	for(int i = 0 ; i < fTime[0].size()  ; i++ )
		cout << "\t\033[36m" << i << "\033[0m\t" << fTime[0][i]/(3600.*24*365.25)<<endl;
	cout << endl;
	cout << "│↑↑↑↑↑↑↑↑↑↑↑↑ READ ABOVE ↑↑↑↑↑↑↑↑↑↑↑↑↑│" << endl; 
	cout << " Enter time step : " << endl;
	GetUserValue(fKeffStep);
	cout << "The keff at time step : " << fKeffStep << "("<<fTime[0][fKeffStep]<<"year) will be used to predict fissile content" << endl ;
	cout << endl;
    ...
    cout << "│ move this folder to $CLASS_PATH/DATA_BASES/"<<fReactorType<<"/"<<fFuelType << "/EQModel/FBR_Keff/ChooseAName  │" << endl; 
	cout << "╰────────────────────────────────────────────────╯" << endl; 
	cout << endl; 
	...
	if(UserSayYes())
	{
		...
		{	
			system("./train");
			if(TrainingHasSucceed())
			{		
				cout << "\t \033[32m  CONGRATULATION TRAINING HAS SUCCEED \033[0m " << endl; 
				cout << "╭────────────────────────────────────────────────╮" << endl; 
				cout << "│                  NEXT STEPS:                   │" << endl; 
				cout << "├────────────────────────────────────────────────┤" << endl; 
				...
				cout << "│	     \033[36mUtils/XSM/MLP/Test/EvaluateTrainingCommands.dat\033[0m│" << endl; 
				cout << "│2. Put the file .nfo in \033[36mweights\033[0m folder then      │" << endl; 
    			cout << "│ move this folder to $CLASS_PATH/DATA_BASES/"<<fReactorType<<"/"<<fFuelType << "/EQModel/FBR_Keff/ChooseAName  │" << endl; 
				cout << "╰────────────────────────────────────────────────╯" << endl; 
			}
			else
			{
				cout << "\t \031[32m TRAINING HAS FAILED \033[0m" << endl;
				cout << "I supposed you did something nasty. "    << endl;
				cout << "Hints : "<<endl;
				cout << "Does your training sample is big enough ?" << endl;
				cout << "-> Look for \"ANN too complicated:\" in TMVA term output " <<endl,
				cout << " If present maybe you can increase the proportion of events for training" << endl;
				cout << " or change ANN archintecture in Train.cxx " <<endl;
			}
...
//--------------------------------------------------------------------------------------------------
bool TrainingHasSucceed()
{
	if(is_file_exist("weights/TMVARegression_MLP_k_eff.weights.xml" ))
		return true;
...
string CreateInfoFile()
{
	...
	InfoFile << "To be used with : EQM_FBR_MLP_Keff.cxx "    << endl;
	InfoFile <<" THIS MODEL PREDICT FISSILE CONTENT THANKS TO Keff CALCULATES AT :"<<endl;
	InfoFile <<" Time Step : # " << fKeffStep << " which correspond to" << fTime[0][fKeffStep]<< " year(s)" <<endl;
  	InfoFile <<"=================================================" << endl;
  	InfoFile <<"  Informations needed by EQM_FBR_MLP_Keff model    " << endl;
  	InfoFile <<"=================================================" << endl;
  	InfoFile << endl;
