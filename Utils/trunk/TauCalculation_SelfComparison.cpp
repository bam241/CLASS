...
int main(int argc, char** argv)
{
	...
	for( it = evolutiondataBase.begin(); it != evolutiondataBase.end(); it++ )
	{
		...
		EvolutionData EVOBuild = MOX.GenerateEvolutionData( (*it).second.GetIsotopicVectorAt(0.).GetActinidesComposition() ,it->second.GetFinalTime() , it->second.GetPower());//*(1+9.72932285999612498e-03));
...
//			   && it_Q_Build != QuantityBuild.end() && it_Fiss_Build != XSFissionBuild.end() && it_Cap_Build != XSCaptureBuild.end() && it_n2n_Build != XSn2nBuild.end() )
			if( it_Q_Build != QuantityBuild.end() )
				for (int i= 0; i < 10; i++)
				{
...
/*
			...
			Tau_Phi[i]->Fill(TauPhi[i]);
			Tau_N[i]->Fill(TauN[i]);

			TauPhiTot[i] += TauPhi[i];
			TauNTot[i] += TauN[i];
*/
		}
	...
	/*	TGraph* Tau_PhyTot= new TGraph( 10, T, TauPhiTot);
	name = "Mean_Tau_Phi";
	Tau_PhyTot->SetName(name.c_str());
	Tau_PhyTot->Write();

	TGraph* Tau_NTot= new TGraph( 10, T, TauNTot);
	name = "Mean_Tau_N";
	Tau_NTot->SetName(name.c_str());
	Tau_NTot->Write();

	TGraph* Tau_NTot_Nico= new TGraph( 10, T, TauN_Nico);
	name = "Mean_Tau_N_Nico";
	Tau_NTot_Nico->SetName(name.c_str());
	Tau_NTot_Nico->Write();

	TGraph* Tau_NTot_MAX= new TGraph( 10, T, TauN_MAX);
	name = "Tau_NTot_MAX";
	Tau_NTot_MAX->SetName(name.c_str());
	Tau_NTot_MAX->Write();
*/
	outfile->Close();
