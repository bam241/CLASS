...
DecayDataBank::DecayDataBank()
    : CLASSObject(new CLASSLogger("DecayDataBank.log")) {
...
DecayDataBank::DecayDataBank(CLASSLogger* log, string DB_index_file)
    : CLASSObject(log) {
  fDataBaseIndex = DB_index_file;
  fFastCalculation = true;
...
IsotopicVector DecayDataBank::Evolution(const ZAI& zai, double dt) {
  ...
  if (it == fDecayDataBank.end()) {
    ...
    while (!DB_index.eof()) {
      ...
      int rI = atoi(
          StringLine::NextWord(line, start).c_str());  // Get Isomeric State

      if (rZ == zai.Z() && rA == zai.A() && rI == zai.I()) {
        string file_name = StringLine::NextWord(line, start);
        EvolutionData evolutionproduct =
            EvolutionData(GetLog(), file_name, true);
#pragma omp critical(DBupdate)
        {
          fDecayDataBank.insert(
              pair<ZAI, EvolutionData>(zai, evolutionproduct));
        }
    ...
    if (!zaifind) {
      WARNING << " Oups... Can't Find the ZAI : " << zai.Z() << " " << zai.A()
              << " " << zai.I() << "!!! It will be considered as stable !!"
              << endl;
    ...
    returnIV = (*it).second.GetIsotopicVectorAt(dt);
...
IsotopicVector DecayDataBank::GetDecay(IsotopicVector isotopicvector,
  ...
  if (t > 1e16) {
    ERROR << " To long decay ... cut it !!! (max = 1e16 s ~ 300 000 000 years )"
          << endl;
    exit(1);
  }

  if (fFastCalculation)  // If using fast calculation simply interpolate between
                         // 2 timesteps
  {
  ...
  } else  // If using precise calculation calculate the evolution at each digit
  {
    ...
    for (int i = 16; i >= 0; i--) {
      if (evolutionDecade[i] != 0) {
        ...
        IV = IsotopicVector();
        for (it = isotopicquantity.begin(); it != isotopicquantity.end(); it++)
          IV += Evolution(it->first, evolutionDecade[i] * pow(10, i)) *
                (*it).second;
      }
    }
  }
