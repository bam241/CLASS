...
double Distance(IsotopicVector IV1, EvolutionData Evd1) {
  ...
  double SumOfXs = 0;
  for (it = IVtmpIsotopicQuantity.begin(); it != IVtmpIsotopicQuantity.end();
       it++) {
...
EvolutionData operator*(EvolutionData const& evol, double F) {
  ...
  for (it = EvolutionData.begin(); it != EvolutionData.end(); it++) {
    ...
    evoltmp.NucleiInsert(pair<ZAI, TGraph*>(
        (*it).first, new TGraph((*it).second->GetN(), X, Y)));
  }
...
EvolutionData Multiply(EvolutionData const& evol, double F) {
  ...
  for (it = EvolutionData.begin(); it != EvolutionData.end(); it++) {
    ...
    evoltmp.FissionXSInsert(pair<ZAI, TGraph*>(
        (*it).first, new TGraph((*it).second->GetN(), X, Y)));
  }
  ...
  for (it = EvolutionData.begin(); it != EvolutionData.end(); it++) {
    ...
    evoltmp.CaptureXSInsert(pair<ZAI, TGraph*>(
        (*it).first, new TGraph((*it).second->GetN(), X, Y)));
  }
  ...
  for (it = EvolutionData.begin(); it != EvolutionData.end(); it++) {
    ...
    evoltmp.n2nXSInsert(pair<ZAI, TGraph*>(
        (*it).first, new TGraph((*it).second->GetN(), X, Y)));
  }
...
//________________________________________________________________________
EvolutionData::EvolutionData(CLASSLogger* log, string DB_file, bool isDecay,
                             ZAI zai)
    : CLASSObject(log) {
  fIsCrossSection = false;
  fDB_file = DB_file;
...
//________________________________________________________________________
EvolutionData::EvolutionData(bool oldread, CLASSLogger* log, string DB_file,
                             bool isDecay, ZAI zai)
    : CLASSObject(log) {
  fIsCrossSection = false;
  fDB_file = DB_file;
  fPower = 0;
...
void EvolutionData::DeleteEvolutionData() {
  ...
  for (it_del = fInventoryEvolution.begin();
       it_del != fInventoryEvolution.end(); it_del++) {
...
void EvolutionData::AddAsStable(ZAI zai) {
  ...
  fInventoryEvolution.insert(
      pair<ZAI, TGraph*>(zai, new TGraph(2, time, quantity)));
}
...
IsotopicVector EvolutionData::GetIsotopicVectorAt(double t) {
  ...
  map<ZAI, TGraph*>::iterator it;
  for (it = fInventoryEvolution.begin(); it != fInventoryEvolution.end();
       it++) {
...
void EvolutionData::ReadDB(string DBfile, bool oldread) {
  ...
  if (!fisDecay) ReadInfo();  // Read the .info associeted

  ifstream DecayDB(DBfile.c_str());  // Open the File
  if (!DecayDB)                      // check if file is correctly open
  {
    ERROR << " \n Can't open \"" << DBfile << "\"\n" << endl;
    ERROR << "\t-> Hint : If loading .dat files using a .idx file (like for a "
  ...
  for (int i = 0; i < (int)vTime.size(); i++) Time[i] = vTime[i];
  ...
  fHeavyMetalMass =
      cZAIMass.GetMass(GetIsotopicVectorAt(0.).GetActinidesComposition());

  DecayDB.close();
}

void EvolutionData::ReadKeff(string line, double* time, int NTimeStep) {
  if (fKeff != 0) delete fKeff;

  int start = 0;
  if (tlc(StringLine::NextWord(line, start, ' ')) !=
      "keff")  // Check the keyword
  {
...
void EvolutionData::ReadFlux(string line, double* time, int NTimeStep) {
  if (fFlux != 0) delete fFlux;

  int start = 0;

  if (tlc(StringLine::NextWord(line, start, ' ')) !=
      "flux")  // Check the keyword
  {
...
void EvolutionData::ReadInv(string line, double* time, int NTimeStep) {
  ...
  if (A != 0 && Z != 0) {
    ...
    // Add the TGraph
    fInventoryEvolution.insert(
        pair<ZAI, TGraph*>(ZAI(Z, A, I), new TGraph(NTimeStep, time, Inv)));
  }
}
...
void EvolutionData::ReadXSFis(string line, double* time, int NTimeStep) {
  ...
  if (A != 0 && Z != 0) {
    ...
    // Add the TGraph
    fFissionXS.insert(
        pair<ZAI, TGraph*>(ZAI(Z, A, I), new TGraph(NTimeStep, time, XSFis)));
  }
}
...
void EvolutionData::ReadXSCap(string line, double* time, int NTimeStep) {
  ...
  if (A != 0 && Z != 0) {
    ...
    // Add the TGraph
    fCaptureXS.insert(
        pair<ZAI, TGraph*>(ZAI(Z, A, I), new TGraph(NTimeStep, time, XSCap)));
  }
}
...
void EvolutionData::ReadXSn2n(string line, double* time, int NTimeStep) {
  ...
  if (A != 0 && Z != 0) {
    ...
    // Add the TGraph
    fn2nXS.insert(
        pair<ZAI, TGraph*>(ZAI(Z, A, I), new TGraph(NTimeStep, time, XSn2n)));
  }
}
...
void EvolutionData::OldReadDB(string DBfile) {
  ...
  if (tmp == "keff" || tmp == "Keff") {
    ...
    for (int i = 0; i < (int)vKeff.size(); i++) Keff[i] = vKeff[i];
    ...
    if (StringLine::NextWord(line, start, ' ') == "flux") {
      ...
      for (int i = 0; i < (int)vFlux.size(); i++) Flux[i] = vFlux[i];
  ...
  do {
    ...
    if (A != 0 && Z != 0) {
      ...
      for (int k = 0; k < (int)vTime.size(); k++) DPQuantity[k] = 0;

      ZAI zaitmp(Z, A, I);
      int i = 0;
      while (start < (int)line.size()) {
        double DPQuantityTmp =
    ...
    if (line == "Fission") {
      ...
      do {
        ...
        if (A != 0 && Z != 0) {
          ...
          int i = 0;
          while (start < (int)line.size()) {
            long double DPQuantityTmp =
                atof(StringLine::NextWord(line, start, ' ').c_str());
            DPQuantity[i] = (double)DPQuantityTmp;
            i++;
          }
          fFissionXS.insert(pair<ZAI, TGraph*>(
              zaitmp, new TGraph(vTime.size() - 1, Time, DPQuantity)));
        }
        getline(DecayDB, line);
    ...
    if (line == "Capture") {
      ...
      do {
        ...
        if (A != 0 && Z != 0) {
          ...
          fCaptureXS.insert(pair<ZAI, TGraph*>(
              zaitmp, new TGraph(vTime.size() - 1, Time, DPQuantity)));
        }
        getline(DecayDB, line);  // Nuclei is given with "A Z"
...
void EvolutionData::Print(string filename) {
  ...
  if (fFlux) {
    out << "flux ";
    for (int t = 0; t < fFlux->GetN(); t++) out << fFlux->GetY()[t] << " ";
    out << endl;
  }

  if (fKeff) {
    out << "keff ";
    for (int t = 0; t < fKeff->GetN(); t++) out << fKeff->GetY()[t] << " ";
    out << endl;
  }
  for (iterator = fInventoryEvolution.begin();
       iterator != fInventoryEvolution.end(); iterator++) {
    int N = (*iterator).second->GetN();

    if ((*iterator).second->GetY()[N - 1] != 0) {
      out << "Inv " << (*iterator).first.Z() << " " << (*iterator).first.A()
          << " " << (*iterator).first.I() << " ";

      for (int t = 0; t < N; t++) out << (*iterator).second->GetY()[t] << " ";
      out << endl;
    }
  }

  for (iterator = fFissionXS.begin(); iterator != fFissionXS.end();
       iterator++) {
    int N = (*iterator).second->GetN();
    if ((*iterator).second->GetY()[N - 1] != 0) {
      out << "XSFis " << (*iterator).first.Z() << " " << (*iterator).first.A()
          << " " << (*iterator).first.I() << " ";
  ...
  for (iterator = fCaptureXS.begin(); iterator != fCaptureXS.end();
       iterator++) {
    int N = (*iterator).second->GetN();
    if ((*iterator).second->GetY()[N - 1] != 0) {
      out << "XSCap " << (*iterator).first.Z() << " " << (*iterator).first.A()
          << " " << (*iterator).first.I() << " ";
  ...
  for (iterator = fn2nXS.begin(); iterator != fn2nXS.end(); iterator++) {
    int N = (*iterator).second->GetN();
    if ((*iterator).second->GetY()[N - 1] != 0) {
      out << "XSn2n " << (*iterator).first.Z() << " " << (*iterator).first.A()
          << " " << (*iterator).first.I() << " ";
