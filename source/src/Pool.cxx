...
//________________________________________________________________________
Pool::Pool(CLASSLogger* log, cSecond coolingtime)
    : CLASSBackEnd(log, coolingtime, 8) {
  DBGL;
  ...
  INFO << "\t The Cooling Time set at\t " << (double)(fCycleTime / cYear)
       << " year" << endl;
  WARNING << " All Cooled Fuel goes directly to WASTE after cooling !! "
          << endl;
...
//________________________________________________________________________
Pool::Pool(CLASSLogger* log, CLASSBackEnd* storage, cSecond coolingtime)
    : CLASSBackEnd(log, coolingtime, 8) {
  DBGL;
...
void Pool::SetIVArray(vector<IsotopicVector> ivarray) {
  ...
  INFO << "Use SetIVArray(vector<IsotopicVector> ivarray, vector<cSecond>n "
          "timearray) unstead!!!!"
       << endl;
}
...
#pragma omp parallel for
  for (int i = 0; i < (int)fIVArray.size(); i++) {
    if (abs(t - fIVArrayArrivalTime[i] - fCycleTime) <
        3600)  // ">" should not append, only " = " is normal...
    {
      if (t - fIVArrayArrivalTime[i] > fCycleTime)  // Warning & Quit
      {
        ERROR << " Cooling Step : " << t / cYear << " :"
      ...
      RemainingCoolingTime =
          fCycleTime - (fInternalTime - fIVArrayArrivalTime[i]);
      // Cooling Decay
...
void Pool::Dump() {
  DBGL;
  //------ Cooling ------//
  for (int i = (int)fCoolingEndOfCycle.size() - 1; i >= 0;
       i--)  // IV End Of Cooling
  {
    ...
    fCoolingEndOfCycle.erase(fCoolingEndOfCycle.begin() +
                             i);  // Remove index entry
