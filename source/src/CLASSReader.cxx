...
CLASSReader::CLASSReader() : freader(new TMVA::Reader("silent")) { ; }
...
CLASSReader::CLASSReader(const std::vector<std::string>& names)
  ...
  for (std::vector<std::string>::const_iterator v_it = names.begin();
       v_it != names.end(); ++v_it, ++l_it) {
    freader->AddVariable(v_it->c_str(), &(*l_it));
  }
}

//____________________________________________________________________________
CLASSReader::~CLASSReader() { delete freader; }
...
//____________________________________________________________________________
TMVA::IMethod* CLASSReader::BookMVA(const std::string& methodTag,
                                    const std::string& weightfile) {
  return freader->BookMVA(methodTag.c_str(), weightfile.c_str());
}

//____________________________________________________________________________
const std::vector<float>& CLASSReader::EvaluateRegression(
