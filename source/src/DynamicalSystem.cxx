...
//________________________________________________________________________
void DynamicalSystem::RungeKutta(double *YStart, double t1, double t2,
                                 int EquationNumber) {
  ...
  if (fabs(t1 - t2) <= TINY) cout << "Integration time is 0." << endl;
  ...
  for (nstp = 1; nstp <= MAXSTP; nstp++) {
    ...
    if ((t - t2) * (t2 - t1) >= 0.0) {
      ...
      delete[] y;
      delete[] yscal;
      // cout << "The maximum step used in RK was " << fMaxHdid << " Step NUM in
      // RK was " << nstp << endl;
      return;
    }
    if (fabs(hnext) <= fHmin)
...
//________________________________________________________________________
void DynamicalSystem::RK4(double *y, double *dydx, double x, double h,
                          double *yout) {
  ...
  for (int i = 0; i < fNVar; i++) {
    ...
    if (!fIsNegativeValueAllowed && yout[i] < 0) {
      ...
      // << "i = " << i << " ("/* <<
      // fEvolvingMaterial->GetComposition()[i]->GetZAI()->PrintName()
      //		*/ << ") old proportion = " << y[i] << " new = " <<
      // yout[i]
      // << ". Setting to 0." << endl;
      yout[i] = 0.;
    }
...
//________________________________________________________________________
void DynamicalSystem::AdaptStepSize(double *y, double *dydx, double *x,
  ...
  for (;;) {
    ...
    *x = xsav + h;
    if (*x == xsav) {
      // cout << "Step size (" << h << ") too small in routine AdaptStepSize" <<
      // endl;
    }
