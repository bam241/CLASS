...
IsotopicVector::IsotopicVector(IsotopicVector const& IVa)
    : fIsotopicQuantity(IVa.fIsotopicQuantity),
      fIsotopicQuantityNeeded(IVa.fIsotopicQuantityNeeded) {
  ;
}
...
bool IsotopicVector::operator<(const IsotopicVector& isotopicvector) const {
  ...
  } else if ((*this).GetIsotopicQuantity().size() !=
             isotopicvector.GetIsotopicQuantity().size()) {
    return (*this).GetIsotopicQuantity().size() <
           isotopicvector.GetIsotopicQuantity().size();
  } else {
    map<ZAI, double>::iterator it;
    map<ZAI, double>::iterator it2 =
        isotopicvector.GetIsotopicQuantity().begin();
...
void IsotopicVector::Multiply(double factor) {
  ...
  for (it = fIsotopicQuantityNeeded.begin();
       it != fIsotopicQuantityNeeded.end(); it++) {
    (*it).second = (*it).second * factor;
  }
}
...
double IsotopicVector::GetSumOfAll() const {
  return std::accumulate(
      ...
      [](const double sum, const std::pair<ZAI, double>& zaiQ) {
        return zaiQ.second + sum;
      });
}
  ...
  if (quantity > 0) {
    if (it != fIsotopicQuantity.end()) {
      if (it->second > quantity) {
        ...
        if ((it->second - quantity) / it->second >
            1e-16)  // to fit with double precision : 16 digits
        {
...
void IsotopicVector::ApplyZAIThreshold(int z) {
  ...
  while (it != fIsotopicQuantity.end()) {
    if (it->first.Z() < z) {
      quantity += it->second;
      it = fIsotopicQuantity.erase(
          it++);  // becarefull of postcrement incrementation
    } else {
      ++it;
    }
  }
...
void IsotopicVector::Need(const ZAI& zai, double quantity) {
  ...
  if (quantity > 0) {
    cout << "Negative quantity : " << quantity << " for ZAI " << zai.Z() << " "
         << zai.A() << " " << zai.I() << " in this IsotopicVector" << endl;
    exit(0);
    IResult = fIsotopicQuantityNeeded.insert(pair<ZAI, double>(zai, quantity));
    if (!IResult.second) IResult.first->second += quantity;
  }
}
...
//________________________________________________________________________
double IsotopicVector::GetZAIIsotopicQuantity(const int z, const int a,
                                              const int i) const {
...
//________________________________________________________________________
IsotopicVector IsotopicVector::GetThisChemicalComposition(
    IsotopicVector IVa) const {
  IsotopicVector tmp;
  vector<int> ZList;
  ...
  for (int i = 0; i < ZList.size(); i++) {
    for (const_iterator jt = fIsotopicQuantity.begin();
         jt != fIsotopicQuantity.end(); jt++)
...
double IsotopicVector::GetMeanMolarMass() const {
  return GetTotalMass() * 1e6 * AVOGADRO /
         GetActinidesComposition().GetSumOfAll();
}
...
vector<ZAI> IsotopicVector::GetZAIList() const {
  ...
  std::transform(fIsotopicQuantity.begin(),
                 fIsotopicQuantity.end(),  // input iterator
...
void IsotopicVector::Write(string filename, cSecond time) const {
  ...
  if (time != -1) IVfile << "Time " << time / cYear << endl;
...
//________________________________________________________________________
void IsotopicVector::Print(string option) const { cout << sPrint(); }
//________________________________________________________________________
string IsotopicVector::sPrint() const {
  ...
  if (QuantityPrint) {
    ...
    for (it = IsotopicQuantityNeeded.begin();
...
IsotopicVector operator/(IsotopicVector const& IVA, IsotopicVector const& IVB) {
  ...
  if (IVAFromB_Size < IVA_Size) {
    ...
    cout << " IVA / IVB : Size of IVB is bigger than IVA. Non commun nuclei "
            "will not be present in IVresult"
         << endl;
...
double Distance(const IsotopicVector& a, const IsotopicVector& b,
                int DistanceType, const IsotopicVector& DistanceParameter) {
  if (DistanceType == 0) {
    ...
    std::cout
        << " DistanceType defined by the user isn't recognized by the code"
        << std::endl;
...
//____________________________________________________________________________
double DistanceAdjusted(const IsotopicVector& a, const IsotopicVector& b,
...
//____________________________________________________________________________
double Norme(const IsotopicVector& a, int DistanceType,
             const IsotopicVector& DistanceParameter) {
  IsotopicVector zero;
