...
//________________________________________________________________________
FabricationPlant::FabricationPlant(CLASSLogger* log, double fabricationtime)
    : CLASSFacility(log, fabricationtime, 16) {
  DBGL;
  SetName("F_FabricationPLant.");
  ...
  INFO << "\t Fabrication time set to \t " << (double)(GetCycleTime() / cYear)
       << " year" << endl
       << endl;
  DBGL;
}
...
void FabricationPlant::SetSeparationEfficiency(IsotopicVector IV,
  ...
  fSeparationStrategy.insert(
      pair<cSecond, IsotopicVector>(TimeOfSep, SeparationEfficiency));
}

//________________________________________________________________________
IsotopicVector FabricationPlant::GetSeparationEfficiencyAt(cSecond time) {
  DBGV("Getting Separation Efficiency at time : "
       << (time / 3600 / 24. / 365.25) << " y")
...
void FabricationPlant::FabricationPlantEvolution(cSecond t) {
  ...
  for (it = fReactorNextStep.begin(); it != fReactorNextStep.end(); it++) {
    double R_CreactionTime =
        GetParc()->GetReactor()[(*it).first]->GetCreationTime();
    double R_LifeTime = GetParc()->GetReactor()[(*it).first]->GetLifeTime();
    int ReactorId = (*it).first;
    ScheduleEntry* R_Entry =
        GetParc()->GetReactor()[ReactorId]->GetScheduler()->GetEntryAt(
            t + GetCycleTime());
    ...
    if (R_CycleTime < GetCycleTime()) {
      ERROR << "Reactor Cycle Time is shorter than Fabrication Time of the "
               "fuel, we cannot deal it upto now!!!"
            << endl;
      ERROR << "Time : " << t / 24. / 3600. / 365.25 << endl;
      ERROR << "Reactor : " << GetParc()->GetReactor()[ReactorId]->GetName()
            << endl;
      ERROR << "Reactor Cycle Time: " << R_CycleTime / 24. / 3600. / 365.25
            << endl;
      ERROR << "Fabrication Plant Cycle Time : "
            << GetCycleTime() / 24. / 3600. / 365.25 << endl;
      exit(1);
    }
...
#pragma omp critical(FuelBuild)
      ...
      } else if ((*it).second - R_CycleTime + GetCycleTime() >= t &&
                 (*it).second - R_CycleTime < t) {
        map<int, IsotopicVector>::iterator it2 =
            fReactorFuturIV.find((*it).first);
...
void FabricationPlant::BuildFuelForReactor(int ReactorId, cSecond t) {
  ...
  for (it_s_IV = fStreamList.begin(); it_s_IV != fStreamList.end(); it_s_IV++) {
    ...
    if (find(LambdaArray[StreamName].begin(), LambdaArray[StreamName].end(),
             -1) != LambdaArray[StreamName].end()) {
  ...
  for (it_s_B = fErrorOnLambda.begin(); it_s_B != fErrorOnLambda.end();
       it_s_B++) {
    if (fErrorOnLambda[(*it_s_B).first]) {
      fFuelCanBeBuilt = false;
    }
  }
  if (fFuelCanBeBuilt && LambdaSum > 0) {
    ...
    EvolutionData EvolDB = FuelType->GenerateEvolutionData(
        GetDecay(IV, fCycleTime), R_CycleTime, R_Power);
    {
      pair<map<int, IsotopicVector>::iterator, bool> IResult;
      IResult =
          fReactorFuturIV.insert(pair<int, IsotopicVector>(ReactorId, IV));
      if (!IResult.second) {
    ...
    }
    {
      ...
      IResult =
          fReactorFuturDB.insert(pair<int, EvolutionData>(ReactorId, EvolDB));
      if (!IResult.second) {
    ...
    // If the building fails : possibility to take a materials from an infinite
    // stock composed of a substitution IV defined by user
    bool IsSubstitutionMaterials = false;
    for (it_s_B = fErrorOnLambda.begin(); it_s_B != fErrorOnLambda.end();
         it_s_B++) {
      if (fErrorOnLambda[(*it_s_B).first] &&
          fSubstitutionMaterialFromIV[(*it_s_B).first]) {
        IsSubstitutionMaterials = true;
      } else if (fErrorOnLambda[(*it_s_B).first] &&
                 !fSubstitutionMaterialFromIV[(*it_s_B).first]) {
    ...
    if (IsSubstitutionMaterials) {
      ...
      for (it_s_B = fSubstitutionMaterialFromIV.begin();
           it_s_B != fSubstitutionMaterialFromIV.end(); it_s_B++) {
        CooledSeparatedIV[(*it_s_B).first] =
            GetDecay(fSubstitutionIV[(*it_s_B).first], GetCycleTime());
      }
      for (it_s_vIV = fStreamArray.begin(); it_s_vIV != fStreamArray.end();
           it_s_vIV++) {
        if (fErrorOnLambda[it_s_vIV->first]) {
          fStreamArray[it_s_vIV->first].clear();
          fStreamArray[it_s_vIV->first].push_back(
              CooledSeparatedIV[(*it_s_vIV).first]);
        }
      }
      // Building the fuel :
      for (it_s_vD = LambdaArray.begin(); it_s_vD != LambdaArray.end();
           it_s_vD++) {
      ...
      EvolutionData evolutiondb =
          fSubstitutionEvolutionData * R_HM_Mass / IV.GetTotalMass();
      ...
      {
        pair<map<int, EvolutionData>::iterator, bool> IResult;
        IResult = fReactorFuturDB.insert(
            pair<int, EvolutionData>(ReactorId, evolutiondb));
        if (!IResult.second) {
      ...
      DBGV("No Alternative Solution. Reactor not loaded. ") {
        ...
        pair<map<int, EvolutionData>::iterator, bool> IResult;
        IResult = fReactorFuturDB.insert(
            pair<int, EvolutionData>(ReactorId, EmptyDB));
        if (!IResult.second) {
      ...
      }
      {
        ...
        pair<map<int, IsotopicVector>::iterator, bool> IResult;
        IResult = fReactorFuturIV.insert(
            pair<int, IsotopicVector>(ReactorId, EmptyIV));
        if (!IResult.second) {
...
void FabricationPlant::BuildArray(int ReactorId, cSecond ReactorLoadingTime) {
  ...
  for (it = fStreamList.begin(); it != fStreamList.end(); it++) {
    if (fInfiniteMaterialFromList[(*it).first]) {
      IsotopicVector IV =
          (*it).second / (*it).second.GetTotalMass() * R_HM_Mass;
      ...
      for (int j = 0; j < (int)fStorage[(*it).first].size(); j++) {
        ...
        for (int k = 0; k < (int)IVArray.size(); k++) {
          IsotopicVector SeparatedIV =
              Separation(IVArray[k], (*it).second).first;
          if (Norme(SeparatedIV) != 0) {
            IsotopicVector CooledSeparatedIV =
                GetDecay(SeparatedIV, GetCycleTime());
            StreamArray.push_back(CooledSeparatedIV);
            StreamArrayAdress.push_back(pair<int, int>(j, k));
            StreamArrayTime.push_back(
                fStorage[(*it).first][j]->GetIVArrayArrivalTime()[k]);
          }
        }
      }
    }
    fStreamArray[(*it).first] = StreamArray;
    StreamArray.clear();
    fStreamArrayAdress[(*it).first] = StreamArrayAdress;
    StreamArrayAdress.clear();
    fStreamArrayTime[(*it).first] = StreamArrayTime;
    StreamArrayTime.clear();
  }
  SortArray();
  DBGL;
}
...
void FabricationPlant::SortArray() {
  ...
  for (it_s_vIV = fStreamArray.begin(); it_s_vIV != fStreamArray.end();
    ...
    if ((int)IVArray.size() >
        1)  // no need to sort if there is only one IV in stock !!
    {
      switch (fStorageManagement) {
        case kpFiFo:
          SortFiFo(IVArray, TimeArray, AdressArray);
          break;
        case kpLiFo:
          SortLiFo(IVArray, TimeArray, AdressArray);
          break;
        case kpMix:
          SortMix(IVArray, TimeArray, AdressArray);
          break;
        case kpRand:
          SortRandom(IVArray, TimeArray, AdressArray);
          break;
        default:
          ERROR << " Posibble Storage Management are" << endl;
          ERROR << " YourFabPlant->SetStorageManagement(key); //with key can "
                   "be either"
                << endl;
          ERROR << "\tkFiFo : First In First Out (i.e the older storage first)"
                << endl;
          ERROR << "\tkLiFo : Last In First Out  (i.e the youger storage first)"
                << endl;
          ERROR << "\tkMix : IVs are sorted that way : "
                << "\n"
                << "First: The younger , Second: The older, Third: The second "
                   "younger ,4th : the second older ...."
                << endl;
...
void FabricationPlant::SortMix(vector<IsotopicVector>& IVArray,
  ...
  if ((double)IVsize / 2. - (double)HalfSize > 0.0)  // if odd number of
                                                     // isotopic vector : one
                                                     // isotopic vector is still
                                                     // missing add it @ the end
  {
...
void FabricationPlant::SetSubstitutionFuel(EvolutionData fuel) {
  ...
  double M0 =
      cZAIMass.GetMass(fuel.GetIsotopicVectorAt(0.).GetActinidesComposition());
...
IsotopicVector FabricationPlant::BuildFuelFromEqModel(
    map<string, vector<double> > LambdaArray, double ReactorMass) {
  DBGL;
  ...
  for (it = fStreamList.begin(); it != fStreamList.end(); it++) {
    for (int i = 0; i < (int)fStreamArray[(*it).first].size(); i++) {
      if (fInfiniteMaterialFromList[(*it).first]) {
        BuildedFuel +=
            fStreamArray[(*it).first][i] * LambdaArray[(*it).first][i];
      } else if (fSubstitutionMaterialFromIV[(*it).first] &&
                 fErrorOnLambda[(*it).first]) {
        BuildedFuel +=
            fStreamArray[(*it).first][i] * LambdaArray[(*it).first][i];
      } else {
        if (LambdaArray[(*it).first][i] != 0) {
          ...
          Separated_Lost =
              Separation(fStorage[(*it).first][Stor_N]->GetIVArray()[IV_N] *
                             LambdaArray[(*it).first][i],
                         (*it).second);
  ...
  // With chemical separation the mass of the built fuel is not equal to the
  // wanted mass
  //--> renormalisation to the wanted mass
  // If impurities not considered in the ANN represent more than
  // fImpuritiesTolerance of the mass there is a crash
  ...
  if (WantedMass != 0) {
    ...
    if (abs(RelativeDifference) < fImpuritiesTolerance) {
      ...
      ERROR << " There are way too many impurities in the stocks : the model "
               "cannot be representative !!!"
            << endl;
    }
  } else {
    ERROR << " No way this fuel can be built : it is supposed to be massfree"
          << endl;
  }
  DBGL;
  return BuildedFuel;
...
void FabricationPlant::DumpStock(map<string, vector<double> > LambdaArray) {
  ...
  for (it = fStreamList.begin(); it != fStreamList.end(); it++) {
    for (int i = 0; i < (int)fStreamArray[(*it).first].size(); i++) {
      if (fInfiniteMaterialFromList[(*it).first]) {
        DBGL;
        GetParc()->AddOutIncome(fStreamArray[(*it).first][0] *
                                LambdaArray[(*it).first][i]);
        DBGL;
      } else {
        if (LambdaArray[(*it).first][i] != 0 && fFuelCanBeBuilt) {
          ...
          int IV_N = fStreamArrayAdress[(*it).first][i].second;
          DBGL;
          fStorage[(*it).first][Stor_N]->TakeFractionFromStock(
              IV_N, LambdaArray[(*it).first][i]);
        }
      }
    }
  }
...
//________________________________________________________________________
pair<IsotopicVector, IsotopicVector> FabricationPlant::Separation(
    IsotopicVector isotopicvector, IsotopicVector ExtractedList) {
  IsotopicVector SeparatedPart;
  IsotopicVector LostPart;
  if (fIsSeparationManagement) {
    //[0] = re-use ; [1] = waste
    IsotopicVector LostInReprocessing =
        isotopicvector.GetThisChemicalComposition(ExtractedList) *
        fSeparationLostFraction;
    SeparatedPart = isotopicvector.GetThisChemicalComposition(ExtractedList) -
                    LostInReprocessing;
  ...
  for (it = isotopicquantity.begin(); it != isotopicquantity.end(); it++) {
    if ((*it).second > 0) {
      IsotopicVector ivtmp =
          fDecayDataBase->Evolution(it->first, t) * (*it).second;
      IV += ivtmp;
    }
  }
  return IV;
...
//________________________________________________________________________
void FabricationPlant::AddStorage(string keyword, Storage* Stock,
                                  double MassFractionMin,
                                  double MassFractionMax, int Priority) {
  fStorage[keyword].push_back(Stock);
  if (MassFractionMin > MassFractionMax) {
    ERROR << " Mass fraction min of material : " << keyword
          << "has to be lower than mass fraction max." << endl;
    exit(1);
  }
  if (MassFractionMin < 0.0) {
    ERROR << " Mass fraction min of material : " << keyword
          << "has to be higher than zero." << endl;
    exit(1);
  }
  if (MassFractionMax > 1.0) {
    ERROR << " Mass fraction max of material : " << keyword
          << "has to be lower than one." << endl;
    exit(1);
  }
  map<int, string>::iterator it_i_s;
  map<string, bool>::iterator it_s_B;
  for (it_i_s = fStreamListFPPriority.begin();
       it_i_s != fStreamListFPPriority.end(); it_i_s++) {
    if ((*it_i_s).first == Priority && (*it_i_s).second != keyword) {
      ERROR << "2 materials have the same position in priority order : "
            << (*it_i_s).second << " and " << keyword << endl;
      exit(1);
    }
  }
  for (it_s_B = fStreamListFPIsBuffer.begin();
       it_s_B != fStreamListFPIsBuffer.end(); it_s_B++) {
    if ((*it_s_B).first == keyword &&
        fStreamListFPIsBuffer[(*it_s_B).first] == 1) {
      ERROR << "The material : " << keyword << " is already defined as buffer. "
            << endl;
      exit(1);
    }
  }
...
//________________________________________________________________________
void FabricationPlant::AddStorage(string keyword, Storage* Stock,
                                  double MassFraction, int Priority) {
  fStorage[keyword].push_back(Stock);
  if (MassFraction < 0.0) {
    ERROR << " Mass fraction of material : " << keyword
          << "has to be higher than zero." << endl;
    exit(1);
  }
  if (MassFraction > 1.0) {
...
//________________________________________________________________________
void FabricationPlant::AddInfiniteStorage(string keyword, double MassFraction,
                                          int Priority) {
  Storage* Stock;
  fStorage[keyword].push_back(Stock);
  if (MassFraction < 0.0) {
...
void FabricationPlant::AddFuelBuffer(string keyword) {
  ...
  for (it_s_B = fStreamListFPIsBuffer.begin();
    ...
    if ((*it_s_B).second == 1) {
      ...
      ERROR << " Fuel can't have more than one buffer in current algorithm. "
            << endl;
      exit(1);
    }
  }
  ...
  for (it_i_s = fStreamListFPPriority.begin();
    ...
    if ((*it_i_s).second == keyword) {
      ERROR << "The material " << keyword
            << " can't be defined as a material with a priority order and a "
               "buffer. "
            << endl;
      exit(1);
    }
