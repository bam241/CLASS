...
//________________________________________________________________________
Storage::Storage(CLASSLogger* log, DecayDataBank* evolutivedb)
    : CLASSBackEnd(log, -1) {
  DBGL;
  SetIsStorageType();
  ...
  if (GetParc()) {
    if (GetParc()->GetStockManagement()) {
      if (fraction > 1 || fraction < 0) {
        WARNING << " You try to remove fraction superior than 1 or a negative "
                   "one..."
                << endl;
      } else {
      ...
      ERROR << " TakeFractionFromStock can't be DEFINE without REAL stock "
               "management"
            << endl;
      exit(1);
    }
    ...
    if (fraction > 1 || fraction < 0) {
      WARNING
          << " You try to remove fraction superior than 1 or a negative one..."
          << endl;
    } else {
      AddCumulativeIVOut(fIVArray[IVId] * fraction);
      fIVArray[IVId] -= fIVArray[IVId] * fraction;
...
void Storage::TakeFromStock(IsotopicVector isotopicvector) {
  if (GetParc()) {
    if (!GetParc()->GetStockManagement()) {
      ...
      ERROR << " TakeFromStock can't be DEFINE WITH REAL stock management"
            << endl;
      exit(1);
    }
  } else {
