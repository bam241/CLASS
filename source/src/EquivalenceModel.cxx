...
void EquivalenceModel::LoadKeyword() {
  DBGL;
  fKeyword.insert(
      pair<string, EQM_MthPtr>("k_zail", &EquivalenceModel::ReadZAIlimits));
  fKeyword.insert(
      pair<string, EQM_MthPtr>("k_reactor", &EquivalenceModel::ReadType));
  fKeyword.insert(
      pair<string, EQM_MthPtr>("k_fuel", &EquivalenceModel::ReadType));
  fKeyword.insert(pair<string, EQM_MthPtr>(
      "k_massfractionmin", &EquivalenceModel::ReadEqMinFraction));
  fKeyword.insert(pair<string, EQM_MthPtr>(
      "k_massfractionmax", &EquivalenceModel::ReadEqMaxFraction));
  fKeyword.insert(
      pair<string, EQM_MthPtr>("k_list", &EquivalenceModel::ReadList));
  fKeyword.insert(pair<string, EQM_MthPtr>(
      "k_specpower", &EquivalenceModel::ReadSpecificPower));
  fKeyword.insert(
      pair<string, EQM_MthPtr>("k_zainame", &EquivalenceModel::ReadZAIName));
  fKeyword.insert(pair<string, EQM_MthPtr>("k_maxburnup",
                                           &EquivalenceModel::ReadMaxBurnUp));
  fKeyword.insert(pair<string, EQM_MthPtr>(
      "k_predictortype", &EquivalenceModel::ReadPredictorType));
  fKeyword.insert(
      pair<string, EQM_MthPtr>("k_output", &EquivalenceModel::ReadOutput));
  fKeyword.insert(
      pair<string, EQM_MthPtr>("k_buffer", &EquivalenceModel::ReadBuffer));
  DBGL;
}
...
void EquivalenceModel::ReadZAIlimits(const string &line) {
  ...
  fZAILimits.insert(pair<ZAI, pair<double, double> >(
      ZAI(Z, A, I), pair<double, double>(downLimit, upLimit)));
  DBGL;
}
//________________________________________________________________________
...
void EquivalenceModel::SetLambdaToErrorCode(vector<double> &lambda) {
  DBGL;
  if (lambda.size() == 0)  // then we have to add an element to send the
                           // error code to the fab (case for no storage in
                           // stream)
  {
    lambda.push_back(-1);
  }
...
void EquivalenceModel::StocksTotalMassCalculation(
    map<string, vector<IsotopicVector> > const &Stocks) {
  DBGL;
...
void EquivalenceModel::ConvertMassToLambdaVector(
    ...
    double MaterialMassNeeded, vector<IsotopicVector> Stocks) {
  DBGL;
  ...
  for (int i = 0; i < (int)lambda.size();
       i++)  // set to 0 all non touched value (to be sure)
    lambda[i] = 0;

  int IntegerPart = floor(Lambda_tot);
  double DecimalPart = Lambda_tot - IntegerPart;

  for (int i = 0; i < IntegerPart; i++) lambda[i] = 1;
...
bool EquivalenceModel::isIVInDomain(IsotopicVector IV) {
  ...
  if (fZAILimits.empty()) {
    ...
    WARNING << "CLASS has no clue if the computed evolution for this fresh "
               "fuel is correct"
            << endl;
    ...
    for (map<ZAI, pair<double, double> >::iterator Domain_it =
             fZAILimits.begin();
      ...
      if ((ThatZAIProp > ThatZAIMax) || (ThatZAIProp < ThatZAIMin)) {
        ...
        WARNING << "\t\t" << Domain_it->first.Z() << " " << Domain_it->first.A()
                << " " << Domain_it->first.I() << endl;
        WARNING << "\t\t min = " << ThatZAIMin << " value = " << ThatZAIProp
                << " max = " << ThatZAIMax << endl;
