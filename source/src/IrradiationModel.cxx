...
/*				Decay Stuff			*/
//________________________________________________________________________
string IrradiationModel::GetDecay(string DecayModes, double& BR, int& Iso,
                                  int& StartPos) {
  string header;

  BR = 0;
  string DecayBR = StringLine::NextWord(
      DecayModes, StartPos, ',');  // extraction of the decay mode and the BR

  int ss = 0;
  string Decay =
      StringLine::NextWord(DecayBR, ss, ':');  // extraction of the decay

  if (ss < (int)DecayBR.size())  // extraction of the BR if exist (i.e. for non
                                 // stable isotop)
...
void IrradiationModel::LoadDecay() {
  ...
  if (!infile) WARNING << " Can't open \"" << DataFullPathName << "\"" << endl;
  ...
  do {
    ...
    if (Z >= fZAIThreshold) {
          ...
          if ((BR > 1e-10) && (!stable)) {
            if (DM <= 15) {
              ...
              if (fSpontaneusYield.size() == 0) {
                ...
                IsotopicVector SpontanuesFissionProduct =
                    fSpontaneusYield.GetFiliation(ParentZAI);
                if (SpontanuesFissionProduct.GetQuantity(-1, -1, -1) == 0) {
                  ...
                  WARNING << " Unknwon Spontanues yield for ZAI : "
                          << ParentZAI.Z() << " " << ParentZAI.A() << " "
                          << ParentZAI.I() << endl;
                  DaughtersIV += 2 * BR * ZAI(-2, -2, -2);
                  branch_test_f += 2 * BR;
                }
      ...
      if (HalfLife < fShorstestHalflife && !stable)
        fFastDecay.Add(ParentZAI,
                       DaughtersIV);  // Fill the FastDecay by the Daughter IV
      else if (stable) {
        fNormalDecay.Add(ParentZAI, IsotopicVector());  // Fill the NormalDecay
                                                        // with a empty IV
                                                        // (mother is stable)
      } else {
        fNormalDecay.Add(ParentZAI, DaughtersIV);  // FIll the NormalDecay with
                                                   // the daughter IV scaled by
                                                   // the decay constante.
...
void IrradiationModel::BuildDecayMatrix() {
  ...
  for (int i = 0; i < (int)fMatrixIndex.size(); i++)
    for (int j = 0; j < (int)fMatrixIndex.size(); j++) fDecayMatrix[i][j] = 0;

  for (int i = 0; i < (int)fReverseMatrixIndex.size(); i++) {
    IsotopicVector DaughterIV =
        fNormalDecay.GetFiliation(fReverseMatrixIndex[i]);
    (*this).GetNuclearProcessMatrix(
        fDecayMatrix, fReverseMatrixIndex[i], DaughterIV,
        fDecayConstante.GetQuantity(fReverseMatrixIndex[i]));
  }
  DBGL;
}

void IrradiationModel::GetNuclearProcessMatrix(
    TMatrixT<double>& NuclearProcessMatrix, ZAI Mother,
    IsotopicVector ProductedIV, double XSValue) {
  DBGL;
  ...
  if (fMatrixIndex.find(Mother) != fMatrixIndex.end()) {
    ...
    for (int j = 0; j < (int)ProductedZAIList.size(); j++) {
      if (fMatrixIndex.find(ProductedZAIList[j]) != fMatrixIndex.end()) {
        NuclearProcessMatrix[fMatrixIndex[ProductedZAIList[j]]][i] +=
            ProductedIV.GetQuantity(ProductedZAIList[j]) * XSValue;
      } else
        NuclearProcessMatrix[0][i] +=
            ProductedIV.GetQuantity(ProductedZAIList[j]) * XSValue;
    }
  } else
    WARNING << " Can't have nuclear process on this nucleus, ZAI " << Mother.Z()
            << " " << Mother.A() << " " << Mother.I()
            << " : its halflife seems to be below the threshold!" << endl;
...
void IrradiationModel::SetFissionEnergy(ZAI zai, double E) {
  ...
  if (!IResult.second) IResult.first->second = E;
}
...
CLASSNucleiFiliation IrradiationModel::ReadFPYield(string Yield) {
  ...
  if (!infile) WARNING << " Can't open \"" << Yield << "\" File !!!" << endl;
  ...
  do {
    ...
    do {
      double Yield_values =
          atof(StringLine::NextWord(line, start, ' ').c_str());
  ...
  for (int i = 0; i < (int)Fissile.size();
       i++)  // Fill the CLASSNucleiFiliation
...
//________________________________________________________________________
void IrradiationModel::LoadFPYield(string SpontaneusYield,
...
void IrradiationModel::NuclearDataInitialization() {
  ...
  if (fSpontaneusYieldFile != "")
    fSpontaneusYield.FiliationCleanUp(
        fMatrixIndex, fFastDecay);  // remove the cutted nuclei....
  if (fReactionYieldFile != "")
    fReactionYield.FiliationCleanUp(
        fMatrixIndex, fFastDecay);  // remove the cutted nuclei....
...
void IrradiationModel::BuildReactionFiliation() {
  ...
  // (n,Gamma) Special Reaction.....
  {
    // 241Am(n,Gamma)
    {
      fCaptureReaction.Add(ZAI(95, 241, 0),
                           ZAI(96, 242, 0) * (1 - BR_AM_242M) *
                               0.827);  // directly cut the Am242 as in MURE
      fCaptureReaction.Add(ZAI(95, 241, 0),
                           ZAI(94, 242, 0) * (1 - BR_AM_242M) *
                               0.173);  // directly cut the Am242 as in MURE
      fCaptureReaction.Add(ZAI(95, 241, 0), ZAI(95, 242, 1) * BR_AM_242M);
    }

    if (fReactionYieldFile != "") {  // 165Ho(n,Gamma)
      {
        fCaptureReaction.Add(ZAI(67, 165, 0),
                             ZAI(68, 166, 0) * (1 - BR_HO_166M));             //
        fCaptureReaction.Add(ZAI(67, 165, 0), ZAI(67, 166, 1) * BR_HO_166M);  //
      }

      // 147Pm(n,Gamma)
      {
        fCaptureReaction.Add(ZAI(61, 147, 0),
                             ZAI(61, 148, 0) * (1 - BR_PM_148M));
        fCaptureReaction.Add(ZAI(61, 147, 0), ZAI(61, 148, 1) * BR_PM_148M);
      }

      // 109Ag(n, Gamma)
      {
        fCaptureReaction.Add(ZAI(47, 109, 0),
                             ZAI(48, 110, 0) * 0.9970 * (1 - BR_AG_110M));
        fCaptureReaction.Add(ZAI(47, 109, 0),
                             ZAI(46, 110, 0) * 0.0030 * (1 - BR_AG_110M));
        fCaptureReaction.Add(ZAI(47, 109, 0), ZAI(47, 110, 1) * BR_AG_110M);
      }

      // 107Ag(n, Gamma)
      {
        fCaptureReaction.Add(ZAI(47, 107, 0),
                             ZAI(48, 108, 0) * 0.9715 * (1 - BR_AG_108M));
        fCaptureReaction.Add(ZAI(47, 107, 0),
                             ZAI(46, 108, 0) * 0.0285 * (1 - BR_AG_108M));
  ...
  for (int i = 2; i < (int)fReverseMatrixIndex.size();
       i++)  // Start at 2 to skeep "TMP" ZAI and "PF" ZAI
  {
    int Z = fReverseMatrixIndex[i].Z();
    int A = fReverseMatrixIndex[i].A();

    if (fCaptureReaction.GetFiliation(fReverseMatrixIndex[i])
            .GetQuantity(ZAI(-1, -1, -1)) == 1) {
      fCaptureReaction.Add(fReverseMatrixIndex[i], ZAI(Z, A + 1) * 1);
    }
    if (fn2nReaction.GetFiliation(fReverseMatrixIndex[i])
            .GetQuantity(ZAI(-1, -1, -1)) == 1) {
      if (A > 1) fn2nReaction.Add(fReverseMatrixIndex[i], ZAI(Z, A - 1) * 1);
    }
  }

  fCaptureReaction.FiliationCleanUp(fMatrixIndex,
                                    fFastDecay);  // clean the filiation link
  fCaptureReaction.NormalizeBranchingRatio();     // normalize it

  fn2nReaction.FiliationCleanUp(fMatrixIndex,
                                fFastDecay);  // clean the filiation link
...
/*				Reaction Stuff			*/
//________________________________________________________________________
TMatrixT<double> IrradiationModel::GetFissionXsMatrix(
    EvolutionData EvolutionDataStep, double TStep) {
  DBGL;
  TMatrixT<double> FissionMatrix =
  ...
  for (it_XS = FissionXS.begin(); it_XS != FissionXS.end();
       it_XS++)  // loop on fissionable nuclei
  {
    ZAI Mother = (*it_XS).first;  // Note the Mother ZAI (not necessary but help
                                  // for reading the code)
    double XS_Value =
        (*it_XS).second->Eval(TStep) * 1e-24;  // Get Cross section values

    IsotopicVector FissionProductIV = fReactionYield.GetFiliation(
        Mother);  // Get the Isotopicvector produced by the reaction

    if (FissionProductIV.GetQuantity(ZAI(-1, -1, -1)) !=
        1)  // Check if ZAI is dealed
      GetNuclearProcessMatrix(
          FissionMatrix, Mother, FissionProductIV,
          XS_Value);  // add the Nuclear process in the Reaction Matrix
    else {
      if (fReactionYieldFile != "")
        WARNING << "Don't have fission Yield for this nuclei, ZAI : "
                << Mother.Z() << " " << Mother.A() << " " << Mother.I() << endl;

      GetNuclearProcessMatrix(
          FissionMatrix, Mother, ZAI(-2, -2, -2) * 2,
          XS_Value);  // add the Nuclear process in the Reaction Matrix
    }
  }
...
//________________________________________________________________________
TMatrixT<double> IrradiationModel::GetCaptureXsMatrix(
    EvolutionData EvolutionDataStep, double TStep) {
  DBGL;
  TMatrixT<double> CaptureMatrix =
      TMatrixT<double>(fReverseMatrixIndex.size(), fReverseMatrixIndex.size());
  for (int i = 0; i < (int)fReverseMatrixIndex.size(); i++)
    for (int j = 0; j < (int)fReverseMatrixIndex.size(); j++)
      CaptureMatrix[i][j] = 0;
  ...
  for (it_XS = CaptureXS.begin(); it_XS != CaptureXS.end();
       it_XS++)  // loop on nuclei
  {
    ZAI Mother = (*it_XS).first;  // Note the Mother ZAI (not necessary but help
                                  // for reading the code)
    double XS_Value =
    ...
    IsotopicVector CaptureProductIV = fCaptureReaction.GetFiliation(
        Mother);  // Get the Isotopicvector produced by the reaction

    if (CaptureProductIV.GetQuantity(ZAI(-1, -1, -1)) !=
        1)  // Check if ZAI is dealed
      GetNuclearProcessMatrix(
          CaptureMatrix, Mother, CaptureProductIV,
          XS_Value);  // add the Nuclear process in the Reaction Matrix
    else
      WARNING << "Can't have capture reaction on this nuclei, ZAI : "
              << Mother.Z() << " " << Mother.A() << " " << Mother.I() << endl;
  }
...
//________________________________________________________________________
TMatrixT<double> IrradiationModel::Getn2nXsMatrix(
    EvolutionData EvolutionDataStep, double TStep) {
  DBGL;

  TMatrixT<double> n2nMatrix =
      TMatrixT<double>(fReverseMatrixIndex.size(), fReverseMatrixIndex.size());
  for (int i = 0; i < (int)fReverseMatrixIndex.size(); i++)
    for (int j = 0; j < (int)fReverseMatrixIndex.size(); j++)
      n2nMatrix[i][j] = 0;
       ...
       it_XS++)  // loop on nuclei
  {
    ...
    IsotopicVector n2nProductIV = fn2nReaction.GetFiliation(
        Mother);  // Get the Isotopicvector produced by the reaction

    if (n2nProductIV.GetQuantity(ZAI(-1, -1, -1)) !=
        1)  // Check if ZAI is dealed
      GetNuclearProcessMatrix(
          n2nMatrix, Mother, n2nProductIV,
          XS_Value);  // add the Nuclear process in the Reaction Matrix
    else
      WARNING << "Can't have n,2n reaction on this nuclei, ZAI : " << Mother.Z()
              << " " << Mother.A() << " " << Mother.I() << endl;
  }
