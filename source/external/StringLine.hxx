...
class StringLine {
 public:
  ...
  /*!
   Find Next word in a line starting from position "start" in the line. If an
   alternative
   separator is given, the word length is defined by the first position of sep
   or alt_sep found.
  ...
  */
  static string NextWord(string Line, int &start, char sep = ' ',
                         char alt_sep = '\0');
  //! Find the previous word in a line.
  /*!
   Find Previous word in a line starting from position "start" in the line. If
   an alternative
   separator is given, the word length is defined by the first position of sep
   or alt_sep found.
  ...
  */
  template <class T>
  static bool ToNumber(T &t, const std::string &s,
  ...
  */
  ...
  /*!
   ...
   \param By : the sub-string  ToReplace is replaced by the sub-string By in
   Inline
  */
...
inline string StringLine::NextWord(string Line, int &start, char sep,
                                   char alt_sep) {
  string Word = "";
...
//_________________________________________________________________________________
inline string StringLine::PreviousWord(string Line, int &start, char sep,
                                       char alt_sep) {
  string Word = "";
  if (start <= 0) {
    return Word;
  ...
  if (alt_sep != '\0') {
    ...
    if (real_pos != pos) real_sep = alt_sep;
  }
...
//_________________________________________________________________________________
inline int StringLine::GetStartWord(string Line, int CurrentPosition, char sep,
                                    char alt_sep) {
  int pos = Line.find(sep, CurrentPosition);
  int alt_pos = -1;
  if (alt_sep != '\0') alt_pos = Line.find(alt_sep, CurrentPosition);
  int real_pos = pos;
  char real_sep = sep;
  if (alt_pos >= 0) {
  ...
  while (CurrentPosition < int(Line.size()) &&
         Line[CurrentPosition] == real_sep)
    CurrentPosition++;
  return CurrentPosition;
}

//_________________________________________________________________________________
inline int StringLine::GetEndWord(string Line, int CurrentPosition, char sep,
                                  char alt_sep) {
  int pos = Line.find(sep, CurrentPosition);
  int alt_pos = -1;
  if (alt_sep != '\0') alt_pos = Line.find(alt_sep, CurrentPosition);
  ...
  if (real_pos == int(string::npos)) return Line.size();
  return real_pos;
}
...
//_________________________________________________________________________________
inline string StringLine::ReplaceAll(string InLine, string ToReplace,
                                     string By) {
  int start = 0;
