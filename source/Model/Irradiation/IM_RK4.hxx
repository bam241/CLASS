...
class IM_RK4 : public IrradiationModel, DynamicalSystem {
  ...
  */
  ...
  void ResetTheMatrix();
  void SetTheMatrix(TMatrixT<double> BatemanMatrix);  //!< Set the Evolution
                                                      //! Matrix (Bateman
  //! equations)
  TMatrixT<double>
  GetTheMatrix();  //!< return the Evolution Matrix (Bateman equations)

  void SetTheNucleiVectorToZero();  //!< Initialize the evolution Matrix
  void ResetTheNucleiVector();
  void SetTheNucleiVector(TMatrixT<double> NEvolutionMatrix);  //!< Set the
                                                               //! Evolution
  //! Matrix
  //!(Bateman
  //! equations)
  TMatrixT<double>
  GetTheNucleiVector();  //!< return the Evolution Matrix (Bateman equations)
                         //@}

 private:
  double *fTheNucleiVector;  //!< The evolving atoms copied from Material
                             //! proportions.
