...
//
//________________________________________________________________________
XSM_MLP::XSM_MLP(string TMVA_Weight_Directory, string InformationFile)
    : XSModel(new CLASSLogger("XSM_MLP.log")) {
  DBGL;
...
//________________________________________________________________________
XSM_MLP::XSM_MLP(CLASSLogger *Log, string TMVA_Weight_Directory,
...
void XSM_MLP::LoadKeyword() {
  ...
  XSModel::LoadKeyword();
  fDKeyword.insert(
      pair<string, XS_MLP_DMthPtr>("k_timestep", &XSM_MLP::ReadTimeSteps));
  fDKeyword.insert(
      pair<string, XS_MLP_DMthPtr>("k_zainame", &XSM_MLP::ReadZAIName));
  DBGL;
}
...
void XSM_MLP::ReadLine(string line) {
  ...
  if (it != fDKeyword.end()) (this->*(it->second))(line);
...
void XSM_MLP::GetMLPWeightFiles() {
  ...
  if (rep == NULL) {
    ERROR << " Reading error for TMVA weight folder  "
          << fTMVAWeightFolder.c_str() << " : " << strerror(errno) << endl;
    exit(1);
  }
  ...
  while ((fichierLu = readdir(rep)) != NULL) {
    ...
    if (FileName != "." && FileName != "..") {
      if (FileName[FileName.size() - 3] == 'x' &&
          FileName[FileName.size() - 2] == 'm' &&
          FileName[FileName.size() - 1] == 'l' && FileName[0] != '.')
...
//________________________________________________________________________
//________________________________________________________________________
void XSM_MLP::ReadWeightFile(string Filename, int &Z, int &A, int &I,
                             int &Reaction) {
  Z = -1;
  A = -1;
  I = -1;
  ...
  if (sReaction == "fis") Reaction = 0;
  if (sReaction == "cap") Reaction = 1;
  if (sReaction == "n2n") Reaction = 2;
...
vector<float> XSM_MLP::CreateTMVAInput(IsotopicVector isotopicvector,
  ...
  IsotopicVector IVAccordingToUserInfoFile =
      isotopicvector.GetThisComposition(IVInputTMVA);
