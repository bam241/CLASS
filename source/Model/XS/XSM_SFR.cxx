...
XSM_SFR::XSM_SFR(string TMVA_Weight_Directory,
                 map<string, double> FixedParameters, string InformationFile) {
...
//________________________________________________________________________
XSM_SFR::XSM_SFR(CLASSLogger* Log, string TMVA_Weight_Directory,
                 map<string, double> FixedParameters, string InformationFile) {
  DBGL;
  fLog = Log;
...
void XSM_SFR::LoadKeyword() {
  ...
  XSM_MLP::LoadKeyword();
  fDKeyword.insert(
      pair<string, XS_SFR_DMthPtr>("k_timestep", &XSM_MLP::ReadTimeSteps));
  fDKeyword.insert(pair<string, XS_SFR_DMthPtr>(
      "k_inputparameter", &XSM_SFR::ReadNonZaiTMVAVariables));
  fDKeyword.insert(
...
vector<float> XSM_SFR::CreateTMVAInput(IsotopicVector IV, int TimeStep) {
  ...
  tmva_input.insert(tmva_input.end(), tmva_input_iv.begin(),
                    tmva_input_iv.end());
  return tmva_input;
}
//________________________________________________________________________
