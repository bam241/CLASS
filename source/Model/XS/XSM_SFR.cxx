...
TTree* XSM_SFR::CreateTMVAInputTree(IsotopicVector isotopicvector,int TimeStep)
{
	...
	for(int i = 0 ; i< (int)fTMVAVariableNames.size() ; i++)
	...
	for(unsigned int j = 0 ; j< fTMVAVariableNames.size() ; j++)
	{
		...
		// Les noms devraients être écrit AAAName*
		// AAA donne le A
		// Name donne le Z
		// * => I=1, pas de * => I=0
		//Maintenant on réccupère les Z, A et I
		int Z(-1),A(-1),I(0);
		//ERROR<<"XXXXXXXX "<<ZAIname<<endl;
		if(ZAIname.back()=='*'){
			I=1;
			ZAIname.pop_back();
		}
		if (isdigit(ZAIname.c_str()[0])){
		...
		//Ensuite on crée les IV
		if(fTMVAFixedVariable[j]==false){
			//ERROR<<"XXXXXXXX "<<Z<<" "<<A<<" "<<I<<" "<<endl;
			//ERROR<<"XXXXXXXX "<<Z<<" "<<A<<" "<<I<<" "<<endl;
	...
	for(unsigned int j = 0 ; j< fTMVAVariableNames.size() ; j++)
	{
		...
		//cout<<"					"<<fTMVAVariableNames[j] << " " << InputTMVA[j]<<endl;
	}
