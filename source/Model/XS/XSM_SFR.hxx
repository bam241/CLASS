...
class XSM_SFR : public XSM_MLP {
 public:
  ...
  /*!
   ...
   \param IsTimeStep : if true , one TMVA weihgt per step time is requiered
   otherwise it assumes time is part of the MLP inputs

   */
  XSM_SFR(string TMVA_Weight_Directory, map<string, double> FixedParameters,
   ...
   */
  XSM_SFR(CLASSLogger *Log, string TMVA_Weight_Directory,
 ...
 private:
  ...
  vector<float> CreateTMVAInput(
      ...
      int TimeStep = 0);  //!<Create input tmva to be read by ExecuteTMVA
  vector<string> fTMVAVariableNames;  //!<  List of TMVA input variable names
                                      //!(read from fMLPInformationFile ) , name
  ...
  bool fIsStepTime;  //!<  true if one TMVA weihgt per step time is requiered
                     //! otherwise it assumes time is part of the MLP inputs
  vector<double> fTMVAFixedVariableValues;  //!< List of value for TMVA that
                                            //! have to be used for all
