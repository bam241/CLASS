...
class IsotopicVector : public TObject {
 public:
  ...
  IsotopicVector& operator+=(
      IsotopicVector const& IVb);  //!< Operator += definition
  IsotopicVector& operator-=(
      IsotopicVector const& IVb);  //!< Operator -=  definition
  IsotopicVector& operator*=(
      IsotopicVector const& IVb);  //!< Operator *=  definition
  IsotopicVector& operator*=(
      double const& factor);  //!< Operator *=  definition (scalar)
  bool operator<(const IsotopicVector& isotopicvector)
      const;  //!< IsotopicVector Comparator
  ...
  IsotopicVector GetSpeciesComposition(
      int z) const;  //!< Return the Species composition of the "z" atom
  IsotopicVector GetThisComposition(IsotopicVector IV)
      const;  //!< Return the composition according the IV list...
  IsotopicVector GetThisChemicalComposition(IsotopicVector IV)
      const;  //!< Return the composition according the IV list...
  vector<ZAI> GetZAIList() const;  //!< Return the list of ZAI present in the IV
  IsotopicVector GetActinidesComposition()
      const;  //!< Return the Actinides composition (Z >= 89)

  double GetZAIIsotopicQuantity(
      const ZAI& zai) const;  ///< Return the quantity of the ZAI
  double GetZAIIsotopicQuantity(const int z, const int a, const int i)
      const;  ///< Return the quantity of the ZAI
  ...
  double GetTotalMass()
      const;  //!< Return the mass (in tons) of the isotopic vector
  double GetMeanMolarMass()
      const;  //<! Return the mean molar mass of the isotopic vector

  vector<int> GetChemicalSpecies()
      const;  //!< Return the list of chemichal species contained
  int GetZAIQuantity() const {
    return fIsotopicQuantity.size();
  }  //!< Return the number of different ZAI in the IsotopicVector

  double GetSumOfAll()
      const;  //!< Return the Sum of nuclei in the IsotopicVector
  ...
  const_iterator find(const ZAI& zai) const {
    return fIsotopicQuantity.find(zai);
  }
  //@}
  ...
  void Add(const ZAI& zai,
           double quantity);  //!< Add Quantity gramme of the ZAI Element
  void Add(const IsotopicVector& isotopicvector);  //!< Add IsotopicVector to
                                                   //!the existing
                                                   //!IsotopicVector
  void Add(const map<ZAI, double>& quantity);  //!< Add IsotopicVector to the
                                               //!existing IsotopicVector
  void Add(int Z, int A, int I, double quantity) {
    (*this).Add(ZAI(Z, A, I), quantity);
  }  //!< Add Quantity gramme of the ZAI Element
  void Add(const pair<ZAI, double>& zaiQ) { Add(zaiQ.first, zaiQ.second); }

  void Need(const ZAI& zai,
            double quantity);  //!< Fill the fIsotopicQuantityNeeded
  void Need(const IsotopicVector&
                isotopicvector);  //!< Fill the fIsotopicQuantityNeeded
  void Need(const map<ZAI, double>& quantityneeded) {
    fIsotopicQuantityNeeded = quantityneeded;
  }
  //!< Fill the fIsotopicQuantityNeeded

  void Remove(const ZAI& zai,
              double quantity);  //!< Remove Quantity gramme of the ZAI Element
  void Remove(const IsotopicVector& isotopicvector);  //!< Remove IsotopicVector
                                                      //!to the existing
                                                      //!IsotopicVector
  ...
  void Write(string filename, cSecond time = -1)
      const;  //!< Write the Content of the IV in the filename file

  void Print(
      string o = " ") const;  //!< Print the composition of the IV in terminal
 ...
 protected:
  map<ZAI, double>
      fIsotopicQuantity;  ///< Isotopic vector composition in atomes Number
  map<ZAI, double>
      fIsotopicQuantityNeeded;  ///< map where negative value are saved
...
double RelativDistance(const IsotopicVector&,
                       const IsotopicVector&);  //!< return the euclidean
                                                //!distance between two IV. The
                                                //!two IV are normalize to unity
double Distance(const IsotopicVector&, const IsotopicVector&,
                int DistanceType = 0,
                const IsotopicVector& DistanceParameter =
                    IsotopicVector());  //!< return weighted euclidean distance
                                        //!between two IV
double DistanceStandard(
    const IsotopicVector&,
    const IsotopicVector&);  //!< return the euclidean distance between two IV
double DistanceAdjusted(const IsotopicVector&, const IsotopicVector&,
                        const IsotopicVector&);  //!< return the weighted
                                                 //!euclidean distance between
                                                 //!two IV
double Norme(const IsotopicVector&, int DistanceType = 0,
             const IsotopicVector& DistanceParameter =
                 IsotopicVector());  //!< return the norm of an IV
