...
class XSModel : public CLASSObject {
 public:
  ...
  /*!
   \name Virtual methods : This following methods are overloaded in the
   derivated classes : XSM_CLOSEST & XSM_MLP & ...
   */
  //@{
  ...
  /*!
   Estimates the mean cross sections evolution according the fresh fuel
   composition
  ...
  void SetZAIThreshold(int Z_Threshold) {
    fZAIThreshold = Z_Threshold;
  }  //!< Set the Z threshold : ZAI with Z < fZAIThreshold are not manage by
     //!CLASS
  int GetZAIThreshold() { return fZAIThreshold; }  //!< Get the Z threshold

 protected:
  bool fread;
  string fInformationFile;  //!<  file containing Reactor Type, Fuel type, HM
                            //!mass, Power, time vector, and TMVA input
                            //!variables names (looks the manual for format
                            //!details)

  double
      fDBPower;  //!<  Power of the data base (read from fMLPInformationFile )
  double fDBHMMass;  //!<  Heavy metal mass of the data base (read from
                     //!fMLPInformationFile )
  string fDBFType;   //!<  Fuel Type    (e.g MOX, UOX, ThU, ThPu ...)
  string fDBRType;   //!<  Reactor Type (e.g PWR, FBR-Na, ADS..)

  map<ZAI, pair<double, double> >
      fZAILimits;  //!< Fresh fuel range : map<ZAI<min edge ,max edge >>

#ifndef __ROOTCLING__
  map<string, XSM_MthPtr> fKeyword;
#endif

  int fZAIThreshold;  //!< Z threshold for handling nuclei mean cross section
                      //!(take only ZAI reaction of Z>= fZAIThresold)
};
