...
class CLASSBackEnd : public CLASSFacility
{
	public :
		...
		std::vector<IsotopicVector> GetIVArray            () const; //!< Return the IsotopicVector Array
		std::vector<cSecond>        GetIVArrayArrivalTime () const; //!<Vector of arrival time of each IV in the CLASSBackEnd
		
		int  GetIVNumber     () const; //!< Return the number of Isotopic Vector present in the CLASSBackEnd object
	...
	protected :
		IsotopicVector GetDecay ( IsotopicVector const& isotopicvector , cSecond t ); //!< Get IsotopicVector decay at time t [s]
...
///// inline functions ///////////////////////////////////////////////////////
inline std::vector<IsotopicVector> CLASSBackEnd::GetIVArray            () const { return fIVArray;            }
inline std::vector<cSecond>        CLASSBackEnd::GetIVArrayArrivalTime () const { return fIVArrayArrivalTime; }
...
{
	if ( i < (int)fIVArray.size() ) { return fIVArray[i];      }
...
inline void CLASSBackEnd::SetIVArray ( std::vector< IsotopicVector > const& ivarray ){ fIVArray = ivarray; }
	...
	inline void CLASSBackEnd::SetOutBackEndFacility ( CLASSBackEnd* befacility )
		{ fOutBackEndFacility = befacility; fIsStorageType = false; }
#endif
