...
//@{
  ...
  void SetIVArray(
      vector<IsotopicVector> ivarray);  //! not use there (does nothing)
  void SetIVArray(vector<IsotopicVector> ivarray,
                  vector<cSecond> timearray);  //!< Set the IsotopicVector Array
  ...
  void Dump();  //!< Write modification (exchange between Cooling, Separation
                //!and Storage)
 ...
 protected:
  ...
  vector<int> fCoolingEndOfCycle;  //!< Index of the cooling IV reaching the end
                                   //!of a cooling cycle
