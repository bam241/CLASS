...
//-----------------------------------------------------------------------------//
//! Stores fuel inventory evolution , mean cross sections evolution, flux
//! evolution, power , ...

/*!
 Define an EvolutionData.
 The aim of these class is to describe the evolution of a single evoluting
 system in CLASS.
 The system can either be a fuel evolution trough irradiation or a nuclei which
 produce, trough its decay, a large nuclei tree.

 The nuclei tree resulting of the evolution are stored in a map of ZAI and
 TGraph, each TGraph correspond to the evolution of the quantity of the
 associeted ZAI.
...
class EvolutionData : public CLASSObject {
 public:
   ...
   */
  EvolutionData(CLASSLogger* log, string DB_file, bool isDecay = false,
                ZAI zai = ZAI(0, 0, 0));
  //}
  ...
  /*!
   ...
   \param zai set the ZAI if you want to add a stable nuclei.
   */
  EvolutionData(bool oldread, CLASSLogger* log, string DB_file,
                bool isDecay = false, ZAI zai = ZAI(0, 0, 0));
  //}
  ...
  /*!
   ...
   One need to call the DeleteEvolutionData() method to fully delete the
   EvolutionData, and then avoiding memory leak...
   */
  ~EvolutionData();
  //}
...
#endif
  void Print(string filename);  //!< Print EvolutionData in a .dat format in a
...
//@{
  ...
  TGraph* GetEvolutionTGraph(
      const ZAI& zai);  //!< return the evolution of the ZAI quantity (TGraph*)

  IsotopicVector GetIsotopicVectorAt(
      double t);  ///< Return the Product IsotopicVector at time t

  double GetHeavyMetalMass() const {
    return fHeavyMetalMass;
  }  //!< Return the heavy metal mass in the core at the begining of the cycle
     //![t]
  ...
  bool NucleiInsert(pair<ZAI, TGraph*> zaitoinsert);     //!< Add a nuclei
                                                         //!evolution to the
                                                         //!evolution map
  bool FissionXSInsert(pair<ZAI, TGraph*> zaitoinsert);  //!< Add a nuclei to
                                                         //!the fission cross
                                                         //!section map
  bool CaptureXSInsert(pair<ZAI, TGraph*> zaitoinsert);  //!< Add a nuclei to
                                                         //!the capture cross
                                                         //!section map
  bool n2nXSInsert(pair<ZAI, TGraph*> zaitoinsert);  //!< Add a nuclei to the
 ...
 protected:
  ...
  bool
      fIsCrossSection;  ///< true if some cross section are present in the database
  bool fisDecay;
  ...
  void ReadDB(string DBfile,
              bool oldread = false);  //!< Main function to read database
  void ReadKeff(string line, double* time,
                int NTimeStep);  //!< Read the Keff in the database
  void ReadFlux(string line, double* time,
                int NTimeStep);  //!< Read the Flux in the database
  void ReadInv(
      string line, double* time,
      int NTimeStep);  //!< Read the Inventory evolution in the database
  void ReadXSFis(string line, double* time,
                 int NTimeStep);  //!< Read the fission cross section evolution
                                  //!in the database
  void ReadXSCap(
      string line, double* time,
      int NTimeStep);  //!< Read the capture cross evolution in the database
  void ReadXSn2n(
      string line, double* time,
      int NTimeStep);  //!< Read the (n,2n) cross evolution in the database
  void ReadInfo();     //!< Read the info file of the database

  double Interpolate(
      double t,
      TGraph&
          EvolutionGraph);  ///< Interpolating the value of EvolutionGraph at the t time

  void AddAsStable(
      ZAI zai);  ///< Use when adding an EvolutionData of a stable nuclei (for "non" decay)
