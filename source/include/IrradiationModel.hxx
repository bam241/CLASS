...
class IrradiationModel : public CLASSObject {
 public:
  ...
  /*!
   ...
   \param XSSet : set of mean cross section to use in order to perform the
   depletion calculation
   \param Power : constant power to use for irradation [W]
   \param irradiationtime : irradiation time [s]
   */
  virtual EvolutionData GenerateEvolutionData(IsotopicVector IV,
  ...
  }  // File name where decay constants and branching ratios are written.
  string GetDataDirectoryName() const {
    return fDataDirectoryName;
  }  //!< Path to fDataFileName

  double GetShorstestHalflife() const {
    return fShorstestHalflife;
  }  //!< Nuclei with HL below fShorstestHalflife are cut (replaced by their
     //!daughter(s))

  void GetNuclearProcessMatrix(TMatrixT<double> &myMatrix, ZAI Mother,
                               IsotopicVector ProductedIV, double XSValue = 1);
  ...
  IsotopicVector GetDecayConstant() const {
    return fDecayConstante;
  }  //!< Get the decay constants
  double GetDecayConstant(
      const ZAI &zai) const;  //!< Get the decay constants of ZAI
  ...
  /*!
   ...
   // \param FissionEnergyFile: filename containing the Fission Energy of some
   nuclei
  ...
  void SetFissionEnergy(int Z, int A, int I, double E) {
    SetFissionEnergy(ZAI(Z, A, I), E);
  }
  //}

  void SetShortestHalfLife(double halflife) {
    fShorstestHalflife = halflife;
  }  //!< Set the Half Life cut
  void SetZAIThreshold(double zaithreshold) {
    fZAIThreshold = zaithreshold;
  }  //!< Set the zai threshold

  void LoadFPYield(string SponfaneusYield,
                   string ReactionYield);  //!< Build Fision Yields maps

  void SetSpectrumType(
      string type);  //!< Set the type of neutron spectrum (thermal or fast)
  ...
  virtual void
  NuclearDataInitialization();  //!< Build Decay matrices & read FpYields if any
  //@}
  ...
  int GetZAIThreshold() {
    return fZAIThreshold;
  }  //!< Gives the threshold (in charge number Z). The nuclei below this
     //!threshold are not managed
  //@}

  //{
  //! Returns a particular decay mode.
  /*!
   \param DecayModes : a list of decay modes with their branching ratios and
   isomeric state of the Daughters.
 ...
 protected:
  double fShorstestHalflife;  //!< Limit on the half life of nuclei to take it
                              //!into account
  ...
  map<ZAI, double> fFissionEnergy;  //!< Store the Energy per fission use for
                                    //!the flux normalisation.

  map<ZAI, int> fMatrixIndex;  //!< correspondance matrix from ZAI to the column
                               //!(or line) of the different Reaction/Decay
                               //!matrix
  vector<ZAI> fReverseMatrixIndex;  //!< correspondance matrix from the column
                                    //!(or line) of the different Reaction/Decay
                                    //!matrix to the ZAI
  ...
  CLASSNucleiFiliation
      fFastDecay;  //!< Store the nuclei being cut (HL threshold)
  CLASSNucleiFiliation fNormalDecay;  //!< Store the uncut nuclei
  IsotopicVector fDecayConstante;     //!< List of decay constants

  CLASSNucleiFiliation
      fSpontaneusYield;                 //!< Store the spontaneus fission yield
  CLASSNucleiFiliation fReactionYield;  //!< Store the reaction fission yield

  CLASSNucleiFiliation
      fCaptureReaction;               //!< Store the reaction capture Filiation
  CLASSNucleiFiliation fn2nReaction;  //!< Store the reaction n,2n Filiation

  string fSpontaneusYieldFile;  //!< Store the name of the spontaneus fission
                                //!yield file
  string fReactionYieldFile;  //!< Store the name of the reaction fission yield
                              //!file

  string fSpectrumType;  //!< Type of the spectrum : thermal or fast. (needed
                         //!for Isomeric branching ratios)

  //{
  /// Return the Fission XS Matrix at the time TStep
  /*!
   // This method extract the fission cross section of an EvolutionData at the
   set time
   // \param EvolutionDataStep : EvolutionData
   // \param TStep : time
   */
  TMatrixT<double> GetFissionXsMatrix(EvolutionData EvolutionDataStep,
                                      double TStep);
  //}
  ...
  /*!
   // This Method extract the capture cross section of an EvolutionData at the
   set time
   // \param EvolutionDataStep : EvolutionData
   // \param TStep :  time
   */
  TMatrixT<double> GetCaptureXsMatrix(EvolutionData EvolutionDataStep,
                                      double TStep);
  //}
  ...
  /*!
   // This Method extract the (n,2n) Cross section of an EvolutionData at the
   set time
   // \param EvolutionDataStep : EvolutionData
   // \param TStep :  time
   */
  TMatrixT<double> Getn2nXsMatrix(EvolutionData EvolutionDataStep,
                                  double TStep);
  //}

  CLASSNucleiFiliation ReadFPYield(
      string
          Yield);  ///< Read a CLASSYield file and return the correpsponding map
