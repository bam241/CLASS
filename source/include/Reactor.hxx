...
/*!
 ...
 The fuel state in the reactor is describe in the IsotopicVector. Its evolution
 is contained in an EvolutionData
...
//}
  ...
  /*!
   ...
   \param CapacityFactor effective charge of the reactor, fraction between 0 &
   1.
   */
  Reactor(CLASSLogger* log, CLASSBackEnd* backend, cSecond creationtime,
  ...
  /*!
   ...
   1.
   */
  Reactor(CLASSLogger* log, FabricationPlant* fabricationplant,
          CLASSBackEnd* backend, cSecond creationtime, cSecond lifetime,
          double Power, double HMMass, double CapacityFactor = 1);
  //}
  ...
  Reactor(CLASSLogger* log, PhysicsModels* fueltypeDB,
          ...
          cSecond creationtime, cSecond lifetime, double Power, double HMMass,
          double BurnUp, double CapacityFactor);
  //}
...
//@{
  ...
  void SetReactorScheduler(ReactorScheduler* reactorscheduler) {
    fReactorScheduler = reactorscheduler;
  ...
  void SetBurnUp(
      double BU);  //!< Set the burnUp reach at end of cycle (Power cte)
  ...
  void SetFabricationPlant(FabricationPlant* FP) {
    fFabricationPlant = FP;
  }  //!< Set the Pointer to the FabricationPlant
  void SetEvolutionDB(EvolutionData evolutionDB);  //!< Set the pointer to the
                                                   //!DB evolution of the
                                                   //!Reactor
#endif
  ...
  void SetNewFuel(
      EvolutionData ivdb);  //!< Change the Evolutive DB of the Reactor
  void CheckListConsistency(PhysicsModels* fueltypeDB,
                            FabricationPlant* fabricationplant);  //!< Check if
                                                                  //!FP/EqM
                                                                  //!lists are
                                                                  //!identical
#ifndef __ROOTCLING__

  void AddScheduleEntry(cSecond time, ReactorModel* Model, double BurnUp,
                        double Power,
                        double HMMass)  //!< Add A new schedule entry
  {
    if (Model->GetPhysicsModels() && !fFabricationPlant) {
      ERROR << "You must add a FabicationPlant to Reactor " << GetName() << "\n"
            << "Use YourReactor->AddFabricationPlant(YourFP);" << endl;
      exit(0);
    }
    fReactorScheduler->AddEntry(time, Model, BurnUp, Power * fCapacityFactor,
                                HMMass);
    fSchedulePowerEvolution.insert(
        pair<cSecond, double>(time, Power * fCapacityFactor));
  ...
  void AddScheduleEntry(cSecond time, EvolutionData* Model, double BurnUp,
                        double Power, double HMMass) {
    fReactorScheduler->AddEntry(time, new ReactorModel(Model), BurnUp,
                                Power * fCapacityFactor, HMMass);
    fSchedulePowerEvolution.insert(
        pair<cSecond, double>(time, Power * fCapacityFactor));
    fScheduleHMMassEvolution.insert(pair<cSecond, double>(time, HMMass));
  }
  ...
  //!< Change the Model and/or BurnUp and/or Power and/or HMMass from time time
  void AddScheduleEntry(cSecond time, PhysicsModels* Model, double BurnUp,
                        double Power, double HMMass) {
    ...
    fSchedulePowerEvolution.insert(
        pair<cSecond, double>(time, Power * fCapacityFactor));
    fScheduleHMMassEvolution.insert(pair<cSecond, double>(time, HMMass));
  }  //!< Change the Model and/or BurnUp and/or Power and/or HMMass from time
     //!time
#endif
 ...
 protected:
  ...
  bool fIsStorage;  //!< true if a storage has been define (to approximate the
                    //!reprocessing using fixed fuel)
  ...
  CLASSBackEnd* fOutBackEndFacility;  //!< Pointer to the BackEnd Facility which
                                      //!collect the spend fuel
  ...
  Storage* fStorage;  //!< Pointer to the Stock (only for reprocessing fuel in
                      //!fixed base...)
  ...
  map<cSecond, double> fSchedulePowerEvolution;   //!<Evolution Of theoritical
                                                  //!Reactor Power following
                                                  //!Scheduler
  map<cSecond, double> fScheduleHMMassEvolution;  //!<Evolution Of theoritical
                                                  //!Reactor HMMass following
                                                  //!Scheduler
