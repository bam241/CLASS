...
class Storage : public CLASSBackEnd {
 public:
  ...
  void TakeFractionFromStock(
      int IVId, double fraction);  //!< Take a part from an IV in sotck;
  void TakeFromStock(
      IsotopicVector isotopicvector);  //!< Take an entire IV from stock

  void AddIV(
      IsotopicVector isotopicvector);  //!< Add an Isotopicvector to the IVArray
  void AddToStock(IsotopicVector isotopicvector) {
    if ((int)isotopicvector.GetIsotopicQuantity().size() > 0)
      AddIV(isotopicvector);
  }  //!< Add an Isotopicvector to the IVArray if it is not empty
  void RemoveEmptyStocks();  //!< delete the empty Isotopicvector(s) contained
                             //!in IVArray
