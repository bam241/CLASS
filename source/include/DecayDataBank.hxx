...
/*!
 ...
 The aim of these class is to describe the evolution of "all" evoluting systems
 in CLASS.

 For the Decay Matrix the DecayDataBank  mainly contains a map of
 <ZAI,EvolutionData>.This map do the correspondance between a ZAI and its decay
 evolution (containing all the daughter nuclei comming from the decay of the
 original ZAI and quantities evolutions).

 @author BaM
 @author Marc
 @author PTO for a part of the Decay management -- steal from MURE (Even if he
 does not kown it!! :))
 @version 2.0
 */
...
class DecayDataBank : public CLASSObject {
 public:
  ...
  /*!
   ...
   \param olfreadmethod : true if the old format of EvolutionData are used
   (deprecated) (ie without the key word such as Inv, XSFiss...)
   */
  ...
  /*!
   ...
   \param DB_index_file : path to the index file
   \param olfreadmethod : true if the old format of EvolutionData are used (ie
   without the key word such as Inv, XSFiss...)
   */
  ...
  /*!
   Use to reset the DecayDataBank to its default values whihout deleting the
   EvolutionData (which contain pointer... ).
  ...
  map<ZAI, EvolutionData> GetDecayDataBank() const {
    return fDecayDataBank;
  }  //!< Return the DecayDataBank
  bool IsDefine(
      const ZAI& zai) const;  //!< True if the key is define, false unstead
  ...
  IsotopicVector Evolution(
      const ZAI& zai,
      double
          dt);  ///< Return the IsotopicVector from the decay of zai during a dt period
