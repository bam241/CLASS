...
/*!
//	A DynamicalSystem is a base class that solves system differential
equations of 1st order.
//	@f[ \frac{d\vec{Y}}{dt} = A\vec{Y}@f]
//	The differential equations are built in DynamicalSystem::BuildEqns ;
the method MUST be
//  defined in the derived classes.
//	In this first version only Runge-Kutta method is implemented, but the
aim of this class
//	is to provide also other methods such as CRAM (Chebyshev rational
...
class DynamicalSystem {
  ...
  */
  ...
  /*!
  For some quantities (such as nuclei composition), negative values are
  forbidden.
  But, due to integration step and very fast variation of the integrated
  variables
  Runge-Kutta wil produce very small negative value. This method is used to
  force
  ...
  */
  ...
  /*!
  // \param YStart: input : the initial condition Y(t1) ; output the final value
  Y(t2)
  ...
  */
  void AdaptStepSize(double *y, double *dydx, double *x, double htry,
                     double eps, double *yscal, double *hdid, double *hnext);

  int fNVar;  //!< The size of the composition vector and /or number of ZAIs
              //!involved.
  ...
  bool
      fIsNegativeValueAllowed;  //!< whether or not negative value are physical.
};
