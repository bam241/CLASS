...
#define ERROR                         \
  if (fLog)                           \
    if (fLog->GetMaxOutPutLVL() >= 0) \
  fLog->E() << "!!!ERROR!!! "         \
            << "[" << __FILE__ << ":" << __FUNCTION__ << "] "
#define WARNING                       \
  if (fLog)                           \
    if (fLog->GetMaxOutPutLVL() >= 1) \
  fLog->W() << "!!WARNING!! "         \
            << "[" << __FILE__ << ":" << __FUNCTION__ << "] "
#define INFO                          \
  if (fLog)                           \
    if (fLog->GetMaxOutPutLVL() >= 2) \
  fLog->I() << "!!!!INFO!!! "         \
            << "[" << __FILE__ << "] "

#define DBGL                                                             \
  if (fLog)                                                              \
    if (fLog->GetMaxOutPutLVL() >= 3)                                    \
      fLog->D() << __FILE__ << " : " << __LINE__ << " [" << __FUNCTION__ \
                << "]" << endl;
#define DBGV(x)                                                            \
  {                                                                        \
    if (fLog)                                                              \
...
class LogType {
 public:
  ...
  void SetSecondOutput(ostream& log) {
    fLog2 = &log;
  }  // used to direct the stream into two output ostream
...
/*!
 ...
 The aim of this class is to centralize the all CLASS software message inside a
 file.
...
class CLASSLogger {
 public:
   ...
   */
  CLASSLogger(string CLASSLoggerName = "CLASS_OUTPUT.log", int VerboseLvl = 0,
              int OutputLvl = 1);
  //}
