...
class EquivalenceModel : public CLASSObject {
 public:
  ...
  /*!
   \param line : line suppossed to contain the ZAI name  starts with
   "k_maxburnup" keyword
   */
  ...
  /*!
   \param line : line suppossed to contain the Specific Power information starts
   with "k_specpower" keyword
   */
  ...
  string GetTMVAXMLFilePath() {
    return fTMVAXMLFilePath;
  }  // Return the path to TMVA XML File path
  string GetTMVANFOFilePath() {
    return fTMVANFOFilePath;
  }  // Return the path to TMVA NFO File path
  void SetTMVAXMLFilePath(string TMVAXMLFilePath) {
    fTMVAXMLFilePath = TMVAXMLFilePath;
  }  // Set the path to TMVA XML File path
  ...
  void ConvertMassToLambdaVector(
 ...
 protected:
  ...
  //@{
  map<string, double>
      fTotalMassInStocks;          //!< Total mass in each vector of stock
