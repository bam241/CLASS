...
class ScheduleEntry : public CLASSObject {
 public:
   ...
   */
  //@{
  ScheduleEntry(ReactorModel* ED_Or_PhysMod, double BurnUp, double Power,
                double HMMass);
  //@}
...
class ReactorScheduler : public CLASSObject {
 public:
  ...
  void AddEntry(cSecond time, ReactorModel* Model, double BurnUp, double Power,
                double HMMass);  //!< Add A new Entry to the scheduler
 ...
 protected:
 private:
  map<cSecond, ScheduleEntry*>
      fReactorSchedulerMap;  ///< Get the reactor scheduler map
